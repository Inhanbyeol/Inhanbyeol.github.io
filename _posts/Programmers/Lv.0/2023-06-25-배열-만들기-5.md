...
> 문제를 직접 풀어보실 경우 [**여기**](https://school.programmers.co.kr/learn/courses/30/lessons/181912){:target="_blank"}를 클릭해 주세요.
{: .prompt-tip }
...
function solution(intStrs, k, s, l) {
    return intStrs.map((x) => Number(x.substring(s,s+l))).filter((x) => k < x);
}
